...
int emummc_storage_init_mmc()
{
	...
	if (!emu_cfg.sector)
	{
		...
		{
			EPRINTF("Apertura cartella eMMC fallita.");
			goto out;
		}
		...
		if (f_stat(emu_cfg.emummc_file_based_path, &fno))
		{
			EPRINTF("Apertura rawnand della emuMMC fallita.");
			goto out;
		}
...
int emummc_storage_read(u32 sector, u32 num_sectors, void *buf)
{
	...
	else
	{
		...
		if (f_open(&fp, emu_cfg.emummc_file_based_path, FA_READ))
		{
			EPRINTF("Apertura immagine della emuMMC fallita.");
			return 0;
		}
		f_lseek(&fp, (u64)sector << 9);
		if (f_read(&fp, buf, (u64)num_sectors << 9, NULL))
		{
			EPRINTF("Lettura immagine della emuMMC fallita.");
			f_close(&fp);
			return 0;
		}
