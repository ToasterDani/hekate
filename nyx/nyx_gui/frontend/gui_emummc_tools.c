...
static void _create_window_emummc()
{
	...
	lv_obj_t *win;
	if (!mbr_ctx.part_idx)
		win = nyx_create_window_custom_close_btn(SYMBOL_DRIVE"  Crea emuMMC a file", _action_emummc_window_close);
	else
		win = nyx_create_window_custom_close_btn(SYMBOL_DRIVE"  Create emuMMC a partizione", _action_emummc_window_close);
...
static void _create_mbox_emummc_raw()
{
	...
	static const char *mbox_btn_format[] = { "\222Continua", "\222Annulla", "" };
	static char *mbox_btn_parts[] = { "\262Parte 1", "\262Parte 2", "\262Parte 3", "\222Annulla", "" };
	...
	{
		s_printf(txt_buf,
			"#C7EA46 Trovata partizione/i valide!#\n"
			"#FF8000 Scegli una partizione per continuare:#\n\n");
	}
	else
		s_printf(txt_buf, "#FFDD00 Non e' stata trovata una partizione valida!#\n\n");

	s_printf(txt_buf + strlen(txt_buf),
		"Tabella Partizioni:\n"
		"#C0C0C0 Parte 0: Tipo: %02x, Inizio: %08x, Dimensione: %08x#\n"
		"#%s Parte 1: Tipo: %02x, Inizio: %08x, Dimensione: %08x#\n"
		"#%s Parte 2: Tipo: %02x, Inizio: %08x, Dimensione: %08x#\n"
		"#%s Parte 3: Tipo: %02x, Inizio: %08x, Dimensione: %08x#",
	...
	if (mbr_ctx.resized_cnt[0] || mbr_ctx.resized_cnt[1] || mbr_ctx.resized_cnt[2])
		strcat(txt_buf, "\n\n#FFDD00 Nota:# Le voci gialle hanno la partizione USER ridimensionata.");

	if (!mbr_ctx.available)
		strcat(txt_buf, "\n#FF8000 Vuoi partizionare la scheda SD?#\n"
						  "#FF8000 (Ti verra' chiesto come procedere)#");
...
static lv_res_t _create_mbox_emummc_create(lv_obj_t *btn)
{
	...
	static const char * mbox_btn_map[] = { "\222File su SD", "\222Partizione su SD", "\222Annulla", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
...
static void _create_emummc_migrated_mbox()
{
	...
	lv_mbox_set_text(mbox,
		"#FF8000 Configurazione emuMMC#\n\n"
		"#96FF00 La configurazione della emuMMC#\n#96FF00 e' stata salvata sulla scheda SD!#");
...
static lv_res_t _create_emummc_migrate_action(lv_obj_t * btns, const char * txt)
{
	...
	static const char *mbox_btn_map1[] = { "\222File su SD", "\222Partizione su SD", "\222Annulla", "" };
	...
	{
		s_printf(txt_buf,
			"#C7EA46 Trovato un backup adeguato per la emuMMC!#\n\n"
			"#FF8000 Vuoi migrarlo?#\n");
		lv_mbox_add_btns(mbox, mbox_btn_map, _create_emummc_mig4_action);
	}
	else if (emummc)
	{
		s_printf(txt_buf,
			"#C7EA46 Trovata emuMMC a partizione su SD!#\n\n"
			"#FF8000 Vuoi riparare il suo config o il tipo di partizione?#\n");
		lv_mbox_add_btns(mbox, mbox_btn_map, _create_emummc_mig3_action);
	}
	else if (em_raw && em_file)
	{
		s_printf(txt_buf,
			"#C7EA46 Trovate sia emunand a file che a partizione esterne!#\n\n"
			"#FF8000 Scegli quale migrare:#\n");
		lv_mbox_add_btns(mbox, mbox_btn_map1, _create_emummc_mig1_action);
	}
	else if (em_raw)
	{
		s_printf(txt_buf,
			"#C7EA46 Trovata emunand a partizione su SD esterna!#\n\n"
			"#FF8000 Vuoi migrarla?#\n");
		lv_mbox_add_btns(mbox, mbox_btn_map, _create_emummc_mig2_action);
	}
	else if (em_file)
	{
		s_printf(txt_buf,
			"#C7EA46 Trovata emunand a file esterna!#\n\n"
			"#FF8000 Vuoi migrarla??#\n");
		lv_mbox_add_btns(mbox, mbox_btn_map, _create_emummc_mig0_action);
	}
	else
	{
		s_printf(txt_buf, "Non sono state trovate emuMMC o emunand esterne!\n");
...
static lv_res_t _create_mbox_emummc_migrate(lv_obj_t *btn)
{
	...
	static char *mbox_btn_map[] = { "\262Backup", "\262Sistema RAW", "\262Emunand", "\222Annulla", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);

	lv_mbox_set_text(mbox,
		"Benvenuto al tool di migrazione della #C7EA46 emuMMC#!\n\n"
		"Scegli che tipo di migrazione vuoi fare.\n"
		"Qualsiasi cosa non trovata avra' il tasto disattivato.");
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_SETTINGS"  Cambia emuMMC");
	lv_win_add_btn(win, NULL, SYMBOL_POWER"  Disattiva", _save_disable_emummc_cfg_action);
...
out0:;
	...
	lv_label_set_static_text(label_txt, "Partizioni grezze su SD");
	...
	for (u32 raw_btn_idx = 0; raw_btn_idx < 3; raw_btn_idx++)
	{
		...
		{
			s_printf(txt_buf, "SD GREZZO %d", raw_btn_idx + 1);
		...
		s_printf(txt_buf, "Inizio settore: 0x%08X\nCartella: %s", emummc_img->part_sector[raw_btn_idx], &emummc_img->part_path[raw_btn_idx * 128]);
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_EDIT"  Gestisci emuMMC");
	...
	lv_obj_t *label_txt = lv_label_create(h1, NULL);
	lv_label_set_static_text(label_txt, "Info & Selezione emuMMC");
	lv_obj_set_style(label_txt, lv_theme_get_current()->label.prim);
	...
	lv_label_set_static_text(label_btn, emu_info.enabled ? "#96FF00 "SYMBOL_OK"  Attivata!#" : "#FF8000 "SYMBOL_CLOSE"  Disattivata!#");
	...
	{
		if (emu_info.sector)
			s_printf(txt_buf, "#00DDFF Tipo:# Partizione grezza su SD\n#00DDFF Settore:# 0x%08X\n#00DDFF Cartella Nintendo:# %s",
				emu_info.sector, emu_info.nintendo_path ? emu_info.nintendo_path : "");
		else
			s_printf(txt_buf, "#00DDFF Tipo:# File su SD\n#00DDFF Cartella base:# %s\n#00DDFF Cartella Nintendo:# %s",
	...
	{
		lv_label_set_static_text(label_txt2, "La emuMMC e' disattivata e la eMMC verra' usata per l'avvio.\n\n");
	}
	...
	lv_label_set_static_text(label_btn, SYMBOL_SETTINGS"  Cambia emuMMC");
	...
	lv_obj_t *label_txt3 = lv_label_create(h2, NULL);
	lv_label_set_static_text(label_txt3, "Strumenti emuMMC");
	lv_obj_set_style(label_txt3, lv_theme_get_current()->label.prim);
	...
	lv_label_set_static_text(label_btn, SYMBOL_DRIVE"  Crea emuMMC");
	...
	lv_label_set_static_text(label_btn, SYMBOL_SHUFFLE"  Migra emuMMC");
